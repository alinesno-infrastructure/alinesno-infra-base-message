   ...
   <div class="app-container">
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="MessageFailList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.MessageFailId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:MessageFail:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.MessageFailId !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:MessageFail:remove']"></el-button>
                     </el-tooltip>
                  </template>
...
import {
   ...
   updateMessageFail,
   addMessageFail
} from "@/api/base/message/failMessage";

const router = useRouter();
const { proxy } = getCurrentInstance();

// 定义变量
const MessageFailList = ref([]);
...
function getList() {
   loading.value = true;
   listMessageFail(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
      MessageFailList.value = res.rows;
...
function handleDelete(row) {
   const MessageFailIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + MessageFailIds + '"的数据项？').then(function () {
      return delMessageFail(MessageFailIds);
   }).then(() => {
      getList();
...
function handleUpdate(row) {
   reset();
   const MessageFailId = row.id || ids.value;
   getMessageFail(MessageFailId).then(response => {
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.MessageFailId != undefined) {
            updateMessageFail(form.value).then(response => {
               proxy.$modal.msgSuccess("修改成功");
               open.value = false;
               getList();
            });
         } else {
            addMessageFail(form.value).then(response => {
