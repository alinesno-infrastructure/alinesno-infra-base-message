   ...
   <div class="app-container">
      <el-row :gutter="20">
         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="MessageHistoryList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.MessageHistoryId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:MessageHistory:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.MessageHistoryId !== 1">
                        <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                           v-hasPermi="['system:MessageHistory:remove']"></el-button>
                     </el-tooltip>
                  </template>
...
<script setup name="MessageHistory">
...
import {
   ...
   addMessageHistory
} from "@/api/base/message/historyMessage";
...
function getList() {
   loading.value = true;
   listMessageHistory(proxy.addDateRange(queryParams.value, dateRange.value)).then(res => {
      loading.value = false;
      MessageHistoryList.value = res.rows;
...
function handleDelete(row) {
   const MessageHistoryIds = row.id || ids.value;
   proxy.$modal.confirm('是否确认删除应用编号为"' + MessageHistoryIds + '"的数据项？').then(function () {
      return delMessageHistory(MessageHistoryIds);
   }).then(() => {
      getList();
...
function handleUpdate(row) {
   reset();
   const MessageHistoryId = row.id || ids.value;
   getMessageHistory(MessageHistoryId).then(response => {
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.MessageHistoryId != undefined) {
            updateMessageHistory(form.value).then(response => {
               proxy.$modal.msgSuccess("修改成功");
               open.value = false;
               getList();
            });
         } else {
            addMessageHistory(form.value).then(response => {
