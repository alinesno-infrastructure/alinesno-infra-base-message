         ...
         <el-col :span="24" :xs="24">
            ...
            <el-table v-loading="loading" :data="MessageList" @selection-change="handleSelectionChange">
               ...
               <el-table-column label="操作" align="center" width="150" class-name="small-padding fixed-width">
                  <template #default="scope">
                     <el-tooltip content="修改" placement="top" v-if="scope.row.MessageId !== 1">
                        <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)"
                           v-hasPermi="['system:Message:edit']"></el-button>
                     </el-tooltip>
                     <el-tooltip content="删除" placement="top" v-if="scope.row.MessageId !== 1">
...
import {
   ...
   updateMessage,
   addMessage
} from "@/api/base/message/sendMessage";
...
function handleDelete(row) {
   ...
   proxy.$modal.confirm('是否确认删除应用编号为"' + MessageIds + '"的数据项？').then(function () {
...
function handleUpdate(row) {
   reset();
   const MessageId = row.id || ids.value;
...
function submitForm() {
   proxy.$refs["databaseRef"].validate(valid => {
      if (valid) {
         if (form.value.MessageId != undefined) {
